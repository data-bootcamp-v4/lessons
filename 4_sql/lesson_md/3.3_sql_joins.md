...
- Learn how to retrieve data from multiple tables based on common columns and how to use the `ON` clause to specify conditions for the join.
- Understand how to utilize aliases for tables and columns.
- Understanding set operations (UNION, INTERSECT, EXCEPT)
...
```
Remember!! The `left table` is the one directly after the `from` and the `right table` the one after the `join`. This, alongside the type of join, determines what information we will retrieve.
```
...
- Determine which table you want to use as `left table` (will go directly after the `from`) and which table will be the `right table` (will go directly after the `join`)
